...
static const int64_t BLOCK_CHAIN_SIZE = 1LL * GB_BYTES;
    ...
    try {
        ...
        if(fs::exists(dataDir))
        {
            if(fs::is_directory(dataDir))
            {
                ...
                replyStatus = ST_OK;
                replyMessage = tr("Add <code>%1name</code> if you intend to create a new directory here.").arg(separator);
            } else {
    ...
    if(!fs::exists(dataDir.toStdString()) || GetBoolArg("-choosedatadir", false))
    {
        ...
        while(true)
        {
            ...
            try {
                ...
                QMessageBox::critical(0, QObject::tr("FlapXcoin"),
